	...
	go func() {
		...
		for _, mdID := range req.MetadataIDs {
			...
			if strings.Contains(tgtMD.Barcode, "-") {
				...
				statusQ := "update hathitrust_statuses set metadata_status=? where metadata_id=?"
				if err := svc.GDB.Exec(statusQ, "skipped", tgtMD.ID).Error; err != nil {
					svc.logError(js, fmt.Sprintf("unable to mark autogenerated metadata %d as skipped: %s", tgtMD.ID, err.Error()))
				}
				continue
			}
			...
			if slices.Contains(generatedCatKeys, tgtMD.CatalogKey) {
				...
				statusQ := "update hathitrust_statuses set metadata_status=? where metadata_id=?"
				if err := svc.GDB.Exec(statusQ, "duplicate", tgtMD.ID).Error; err != nil {
					svc.logError(js, fmt.Sprintf("unable to mark metadata %d as duplicate: %s", tgtMD.ID, err.Error()))
				}
				continue
			}
...
func (svc *ServiceContext) createHathiTrustPackage(c *gin.Context) {
	...
	go func() {
		...
		if len(packagedIDs) > 0 {
			svc.logInfo(js, "Update metadata package created dates and status")
			now := time.Now()
			err = svc.GDB.Model(&hathitrustStatus{}).Where("metadata_id in ?", packagedIDs).
				Updates(hathitrustStatus{PackageStatus: "created", PackageCreatedAt: &now}).Error
			if err != nil {
				svc.logError(js, fmt.Sprintf("Unable to update HathiTrust status records: %s", err.Error()))
			}
